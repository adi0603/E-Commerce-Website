...
if (isset($_POST['buy'])) {
  $_SESSION['view']=$_POST['item'];
  header('Location: buyproduct.php');
}

if (isset($_POST['remove'])) {
  $item=$_POST['item'];
  $result6 = mysqli_query($con,"delete from cart where product_id='$item' and useremail='$email'");
}
      ...
      <main class="page-main">
        <div class="container">
          <table>
            <tbody>
              <form method="POST">
              <tr><th colspan="5">Cart List</th></tr>
              <th>Select</th>
              <th>Product</th>
              <th>Name</th>
              <th>Discription</th>
              <th>Price</th>
              <?php
                $result1 = mysqli_query($con,"select * from cart join product where cart.useremail='$email' and cart.product_id=product.product_id and cart.status='0'");
                if (mysqli_num_rows($result1) > 0) {
                  ...
                  ?>
                    <tr>
                      <td colspan="5"><?php echo "Your cart is empty. Add some products to cart"; ?></td>
                    </tr>
                   <?php
                 }
                if (mysqli_num_rows($result1) > 0) {
