...
if(isset($_POST['submit']))
{
    ...
    $result = mysqli_query($con,'select * from team where email="'.$email.'"');
    if(mysqli_num_rows($result)==0)
    {
      $result = mysqli_query($con,"INSERT into team (name,email,mobile,address) values ('$name','$email','$mobile','$address')");
      $error=1;
    ...
    <?php
        ...
        {?>
            <script type="text/javascript">
                swal("Sorry!", "Memeber already exist!", "error");
            </script>
        <?php
        $error=-1;
        }
